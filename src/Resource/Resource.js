export const TextLoginStatic = {
    titulo: 'Iniciar Sesion',
    email: 'Email',
    contraseña: 'Contraseña',
    btnTitulo: 'Iniciar Sesion',
}
export const TextFormStatic  = {
    titulo: 'Agregar alumno',
    nombres: 'Nombres',
    primerApellido: 'Primer apellido',
    segundoApellido: 'Segundo apellido',
    fechaNacimiento: 'Fecha nacimiento',
    curp: 'Curp',
    grado: 'Grado',
    matricula: 'Matricula',
    cicloEscolar: 'Ciclo escolar',
    btnTitulo: 'Agregar alumno',
}
export const TextCertificatesKC = {
    titulo: 'Constancias emitidas',
    numConst: '#',
    nombreCompleto: 'Nombre completo',
    curp: 'Curp',
    accionesConstancia: 'Acciones constancia',
    imprimir: 'Imprimir',
    verDatosAlumno: 'ver datos',
    eliminar: 'Eliminar',
    actualizar: 'Actualizar'
}
export const TextosStaticosCostancia = {
    secrFederalizada : "SECRETARIA DE EDUCACIÓN FEDERALIZADA",
    secrIndigena: "SECRETARIA DE EDUCACIÓN INDÍGENA",
    jefaturaZona: "JEFATURA DE ZONA 704, IXTACOMITAN",
    zonaEscolar: "ZONA ESCOLAR 401, RAYÓN",
    nombreKinder: 'JARDÍN DE NIÑAS Y NIÑOS “CUAUHTÉMOC”',
    claveCentroTrabajo: "C. C. T. 07DCC1395S, RAYÓN",
    logoAño: ' "2025 AÑO DE ROSARIO CASTELLANOS FIGUEROA,',
    logoAño1: ' POR LA PAZ Y LA JUSTICIA EN CHIAPAS" ',
    tituloCostancia: "CONTANCIA DE ESTUDIO",
    atentamente: 'ATENTAMENTE',
    linea: '__________________________________',
    nombreDirectora: 'Lic. Adela Albina Dominguez Diaz',
    puesto: 'Directora del jardin de niñas y niños CUAUHTÉMOC',
  };
